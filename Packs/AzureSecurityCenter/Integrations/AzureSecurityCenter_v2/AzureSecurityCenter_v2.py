...
# TODO: notice the add_subscription
...
class MsClient:
    ...
    def update_alert(self, resource_group_name, asc_location, alert_id, alert_update_action_type):
        ...
        params = {"api-version": ALERT_API_VERSION}
        return self.ms_client.http_request(method="POST", url_suffix=cmd_url, params=params)
    ...
    # TODO: pay attention to response type and if a change in resp_type is needed
    ...
    # TODO: check url according to conditions.
    ...
    def delete_jit(self, asc_location, resource_group_name, policy_name):
        ...
        self.ms_client.http_request(method="DELETE", url_suffix=cmd_url, params=params)
...
# TODO: spend the time re-write the outputs
...
# TODO: Call this function with return_outputs
...
def main():
    ...
    try:
        ...
        if demisto.command() == "test-module":
            ...
            list_alerts_command(client, demisto.args())
        elif demisto.command() == "azure-sc-update-alert":
            update_alert_command(client, demisto.args())
        elif demisto.command() == "azure-sc-list-location":
            list_locations_command(client)
        elif demisto.command() == "azure-sc-update-atp":
            update_atp_command(client, demisto.args())
        elif demisto.command() == "azure-sc-get-atp":
            get_atp_command(client, demisto.args())
        elif demisto.command() == "azure-sc-update-aps":
            update_aps_command(client, demisto.args())
        elif demisto.command() == "azure-sc-list-aps":
            list_aps_command(client)
        elif demisto.command() == "azure-sc-get-aps":
            get_aps_command(client, demisto.args())
            ...
            list_jit_command(client, demisto.args())
            ...
            list_sc_storage_command(client)
        elif demisto.command() == "azure-list-subscriptions":
            list_sc_subscriptions_command(client)
